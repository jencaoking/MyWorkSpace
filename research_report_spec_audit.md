...
| 英语学习（翻译/单词/录音跟读） | 单词（音标/释义/例句/熟悉度）、SM-2 间隔重复复习流 | **翻译完全未实现**；**录音跟读完全未实现**（无 MediaRecorder、无原音播放、无跟读对比）；单词纯手动录入，不接外部翻译/发音 API |
...
工程已把"本地优先、可离线使用"的主干业务（任务体系、分类、运动含真实计步、健康/记账记录、笔记编辑、番茄钟、后端 CRUD 与基础同步）跑通，可作为可用 MVP。但方案中的"联网与外部服务"一层基本是空壳：翻译、录音跟读、TMDB、天气四类外接 API 均未接入；通知/提醒、跨模块全局搜索、图表可视化、笔记图片、板块开关 UI 也未完成。同时整体技术栈与方案文档不一致（Compose 替代 ViewBinding/Fragment，自研 Markdown 替代 Markwon，无图表库），后端分层与表命名、鉴权、响应字段也与方案描述不符。若要让方案"名副其实"，下一步优先级建议为：① 通知提醒（任务/复诊闹钟）；② 全局 FTS 搜索与板块开关 UI；③ 翻译/录音跟读/影音 TMDB/天气 的外接 API 接入；④ MPAndroidChart 图表与笔记图片上传；⑤ 后端补充 `/sync/full`、LWW 冲突解决、第三方代理，并统一响应字段与表命名。
