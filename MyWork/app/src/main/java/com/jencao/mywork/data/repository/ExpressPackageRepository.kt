...
import com.jencao.mywork.data.local.dao.ExpressPackageDao
...
@Singleton
class ExpressPackageRepository @Inject constructor(private val dao: ExpressPackageDao) : Syncer<ExpressPackageEntity> {
    ...
    override suspend fun mergeRemote(remote: List<ExpressPackageEntity>) = dao.upsertAll(remote)
    ...
    fun observeAll(): Flow<List<ExpressPackageEntity>> = dao.observeAll()
    suspend fun getById(id: String) = dao.getById(id)
    suspend fun insert(item: ExpressPackageEntity) { item.touch(); dao.insert(item) }
    suspend fun update(item: ExpressPackageEntity) { item.touch(); dao.update(item) }
